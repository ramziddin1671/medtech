...
def index(request):
    return render(request, 'fill_data/index.html', {'models': get_models()})
...
def save_data(request, model_slug):
    ...
    category = request.POST.get('category')
    model = [field for field in get_models(
    ) if field['slug'] == model_slug][0]
    ...
    return redirect('fill_data_index')
